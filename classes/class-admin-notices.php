		...
		if ( is_array( $notices['notices'] ) && ! empty( $notices['notices'] ) ) {
			foreach ( $notices['notices'] as $key => $notice ) {
				...
				if ( 'permission-issue' === $notice['type'] ) {
					...
					$extra .= '<small><i>' . __( 'Premia serves files via REST requests, not with direct links. This issue was detected in this link or path' ) . ': ' . esc_url( $notice['data']['file'] ) . '</i></small>';
				}
